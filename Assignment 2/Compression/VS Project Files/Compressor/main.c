...
void transformInput(HuffmanCode* codeMap, uint64 characterCount)
{
	...
	// If we are decoding we set character count to the max for
	//    a uint64 which is 0 - 1.
	if (rFlag) --characterCount;
	...
	for (; count < characterCount; rFlag ? 0 : ++pIn) // <- not allowed an if... ternary works I guess
	{
		if (rFlag)
		{
			if(eofFound)
...
//    modified here instead of on the command line.
	...
	//argc = 7;
	//char* args[] = { "", "-s", "-r", "-o", "../x64/Release/out.md", "-f", "../x64/Release/test.txt" };
	argv = args;
#endif
	...
	Uint8CountMap countMap = { 0 };
	HuffmanTree tree;
	HuffmanCode codeMap[256] = { 0 };

	if (!rFlag)
	{
		countMap = createUint8CountMap(input);
		tree = createUint8HuffmanTree(&countMap);
	...
	if (sFlag)
	{
		...
		{
			printf("----  Input  ----\n");
			printf("File Size Before Decoding : %llu Byte/s\n", (stats.bitsAfterEncoding + stats.dictionaryBitLength + 7) / 8);
			printf("Dictionary Size      : %llu Byte/s", stats.dictionaryBitLength / 8);
			if (stats.dictionaryBitLength % 8 > 0)
				printf(" and %llu Bit/s\n", stats.dictionaryBitLength % 8);
			else
				printf("\n");
			printf("Digest Size          : %llu Byte/s", stats.bitsAfterEncoding / 8);
		...
		else
		{
			...
			if (stats.bitsAfterEncoding % 8 > 0)
				...
				printf("\n");
			printf("Dictionary Size      : %llu Byte/s", stats.encodedDictionaryBits / 8);
			if (stats.encodedDictionaryBits % 8 > 0)
				printf(" and %llu Bit/s\n", stats.encodedDictionaryBits % 8);
			else
				printf("\n");
			...
			if (oFlag)
			{
				uint64 totalBits = stats.bitsAfterEncoding + stats.encodedDictionaryBits;
				printf("Output File Size     : %llu Byte/s", totalBits / 8);
				if (totalBits % 8 > 0)
					printf(" and %llu Bit/s\n", totalBits % 8);
				else
					printf("\n");
				compressionRatio = (double)(totalBits) / (double)(stats.bytesBeforeEncoding * 8);
